...
def order_filter(request):
    ...
    total_revenue = order.aggregate(Sum('total_price'))['total_price__sum'] or 0
    total_products = OrderDetails.objects.filter(order_id__in=order.values_list('order_id', flat=True)).aggregate(Sum('quantity'))['quantity__sum'] or 0
